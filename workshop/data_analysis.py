...
# curve fitting means creating a curve of certain type that is as close as possible to all points of a given data set.
...
plt.figure()
plt.plot(t_coil,h/t_coil,"ro")
y_err = np.sqrt( np.power(1e-3/t_coil,2) + np.power(1e-3 * h/(t_coil**2),2)  )
plt.errorbar(t_coil,h/t_coil,y_err,1e-3)
